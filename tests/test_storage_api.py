...
def _wait_for_current(client, rid: str) -> None:
    body = client.get(f"/api/renderings/{rid}").json()
...
def test_storage_snapshot_lists_per_project_owned_bytes(client, tmp_path: Path) -> None:
    ...
    rid = client.post("/api/renderings", json=_payload(src, name="alpha")).json()["id"]
    _wait_for_current(client, rid)

    snap = client.get("/api/storage").json()
    assert snap["total_bytes"] > 0
    by_id = {p["rendering_id"]: p for p in snap["per_project"]}
    assert rid in by_id
...
def test_two_projects_share_when_inputs_match(client, tmp_path: Path) -> None:
    ...
    src = _make_png(tmp_path / "in.png")
    a_id = client.post("/api/renderings", json=_payload(src, name="alpha")).json()["id"]
    _wait_for_current(client, a_id)
    b_id = client.post("/api/renderings", json=_payload(src, name="beta")).json()["id"]
...
def test_delete_rendering_frees_owned_cache(client, tmp_path: Path) -> None:
    ...
    r = client.delete(f"/api/renderings/{rid}")
    ...
    assert client.get(f"/api/renderings/{rid}").status_code == 404
...
def test_delete_rendering_keeps_shared_cache(client, tmp_path: Path) -> None:
    ...
    client.delete(f"/api/renderings/{a_id}")
    ...
    by_id = {p["rendering_id"]: p for p in post["per_project"]}
...
def test_purge_intermediates_keeps_terminal_outputs(client, tmp_path: Path) -> None:
    ...
    r = client.delete(f"/api/renderings/{rid}/cache?keep_outputs=true")
    assert r.status_code == 200
    body = r.json()
    assert body["evicted_count"] == 0
    # Rendering still exists and its current job is intact.
    assert client.get(f"/api/renderings/{rid}").status_code == 200
