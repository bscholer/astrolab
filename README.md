...
This README is the working design doc. It will move/split as the project grows.
...
- **Platforms**: Linux Mint is the deployment target (always-on desktop running the server). macOS is dev-only. Both must pass tests, but production-grade behavior (systemd unit, file watchers, headless Siril) is Linux-first.
